...
$(document).ready(function() {
 fetchingAndStoringValue = () => {
  ...
  if (person == null || person == "") {
    ...
    for (let i = 0; i < studentnamesandvalues.length; i++) {
      tr = document.createElement('tr');
    var nameandmarks = studentnamesandvalues[i].split(",");
      // let bean = new Bean(nameandmarks[0],nameandmarks[1],nameandmarks[2],nameandmarks[3]);
let bean = new Bean();
bean.studentName=nameandmarks[0];
bean.studentRollNo=nameandmarks[1];
bean.studentStd=nameandmarks[2];
bean.studentSchoolName=nameandmarks[3];
      beanarray[i]=bean;
...
function printingInsertedValues(bean) {
  ...
  function printingValues(beanarray) {
     for (var beanarrayvalue of beanarray){
